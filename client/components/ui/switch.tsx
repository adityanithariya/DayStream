...
>(({ className, loading, ...props }, ref) => (
  <SwitchPrimitives.Root
    className={cn(
      'peer inline-flex h-5 w-9 shrink-0 cursor-pointer items-center rounded-full border-2 border-transparent shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-950 focus-visible:ring-offset-2 focus-visible:ring-offset-white disabled:cursor-not-allowed disabled:opacity-50 data-[state=checked]:bg-secondary data-[state=unchecked]:bg-primary dark:focus-visible:ring-slate-300 dark:focus-visible:ring-offset-slate-950 dark:data-[state=checked]:bg-secondary dark:data-[state=unchecked]:bg-primary',
      className,
    )}
    {...props}
    ref={ref}
  >
    ...
    >
      {loading && (
        <div className="loader !border-secondary scale-50 relative bottom-0.5 right-0.5" />
      )}
    </SwitchPrimitives.Thumb>
  </SwitchPrimitives.Root>
))
